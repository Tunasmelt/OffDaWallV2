...
export async function GET(
  ...
  try {
    ...
    const cacheKey = `release-group-tracks:${mbid}`;
    const cached = getFromCacheWithMeta<{ tracks: Track[] }>(cacheKey);
    if (cached.data) {
    ...
    const releasesResult = await safeCall('musicbrainz', () => MusicBrainz.getReleaseGroupReleases(mbid, 3));
    ...
    const releaseResult = await safeCall('musicbrainz', () => MusicBrainz.getReleaseWithRecordings(releaseMbid));
    const release = releaseResult.ok ? releaseResult.data : null;
    let tracks = release ? extractTracks(release) : [];
    if (releaseResult.ok && !providersUsed.includes('musicbrainz')) {
      providersUsed.push('musicbrainz');
    }

    if (tracks.length === 0 && process.env.LASTFM_API_KEY) {
      const lfResult = await safeCall('lastfm', () => LastFm.getAlbumInfoByMbid(releaseMbid));
    ...
    const responsePayload = { tracks };
    setCache(cacheKey, responsePayload, CACHE_TTL.CATALOG);
    logEvent('info', '[OffDaWallV2] api/release-groups/[mbid]/tracks', { success: true, mbid });
    const resp = respondOk(
      responsePayload,
      { payloadMode: 'preview', providersUsed, cache: { hit: false } }
    );
    ...
    logEvent('error', '[OffDaWallV2] api/release-groups/[mbid]/tracks', { success: false, error: `${error}` });
    return respondError(
      'tracks_failed',
